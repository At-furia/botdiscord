...
bot.on('message', message => {
    ...
    if(!db.get("xp").find({username: msgauthor}).value()){
        ...
        db.get("xp").find({user: msgauthor}).assign({username: msgauthor, xp: userxp[1] += 1}).write();
    ...
    if (!message.channel.permissionsFor(message.member).hasPermission("MUTE_MEMBERS")){
        message.reply("Tu n'as pas le droit de mute ! ;)")
    }else{
        var membermute = message.mentions.users.first();
        console.log(membermute)
        console.log(message.guild.member(membermute).mutable)
        if(!membermute){
            message.reply("L'utilisateur n'existe pas !");
        }else{
            if(!message.guild.member(membermute).mutable){
                message.reply("Utilisateur impossible a mute !");
            }else{
                message.guild.member(membermute).mute().then((member) => {
                message.channel.send(`${member.displayName}a été mute du serveur !`);
            }).catch(() => {
    ...
    if (!message.channel.permissionsFor(message.member).hasPermission("UNMUTE_MEMBERS")){
        message.reply("Tu n'as pas le droit de unmute ! ;)")
    }else{
        var memberunmute = message.mentions.users.first();
        console.log(memberunmute)
        console.log(message.guild.member(memberunmute).unmutable)
        if(!memberunmute){
            message.reply("L'utilisateur n'existe pas !");
        }else{
            if(!message.guild.member(memberunmute).unmutable){
                message.reply("Utilisateur impossible a unmute !");
            }else{
                message.guild.member(memberunmute).mute().then((member) => {
                message.channel.send(`${member.displayName}a été unmute du serveur !`);
            }).catch(() => {
                message.channel.send("Mute Refusé!")
            })
        }
    ...
    if (message.content === prefix + "pc"){
        random();
            ...
            if (randnum == 1){
                message.reply(": Exterminer du titan");
                console.log(randnum);
            }

            if (randnum == 0){
                message.reply(": mort de Marco > Débat !");
                console.log(randnum);
            }
